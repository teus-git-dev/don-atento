  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-3">
            <Box className="text-[var(--color-neon-cyan)]" /> Inspección IA con Visión AR
          </h2>
        ...
        <div className="flex gap-3">
           <button 
             ...
             className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all flex items-center gap-2 ${
               isGhosting ? 'bg-[var(--color-neon-cyan)]/20 text-[var(--color-neon-cyan)] border-[var(--color-neon-cyan)]/40 shadow-[0_0_15px_rgba(0,255,255,0.2)]' : 'bg-white/5 text-gray-500 border-white/10'
             }`}
           >
           ...
           <button 
             ...
             disabled={analyzing}
             className="px-6 py-2 bg-[var(--color-neon-cyan)] text-black font-black uppercase tracking-widest text-[10px] rounded-xl shadow-[0_5px_15px_rgba(0,255,255,0.3)] hover:scale-105 transition-all flex items-center gap-2"
           >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        {/* Main Viewport */}
        <div className="lg:col-span-3 glass rounded-[3rem] border border-white/5 overflow-hidden relative group min-h-[600px]">
           ...
           {!analyzing && findings.map(f => (
             ...
             >
                ...
                {/* Tooltip */}
                <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-xl border border-white/10 p-3 rounded-2xl w-48 opacity-0 group-hover/marker:opacity-100 transition-opacity z-50">
                    <p className="text-[10px] font-bold text-white uppercase mb-1">{f.label}</p>
                    <div className="flex justify-between items-center">
                       <span className="text-[8px] text-gray-500">Confianza: {(f.confidence * 100).toFixed(0)}%</span>
                       <span className="text-[10px] text-[var(--color-neon-cyan)] font-mono">{f.cost}</span>
                    </div>
                </div>
             </div>
           ))}
           ...
           {/* Scanline Effect */}
           {analyzing && (
             <div className="absolute inset-0 bg-[var(--color-neon-cyan)]/5">
                <div className="w-full h-1 bg-[var(--color-neon-cyan)] shadow-[0_0_20px_rgba(0,255,255,0.8)] absolute top-0 animate-[bounce_3s_infinite]" />
             </div>
           )}

           {/* HUD overlay */}
           <div className="absolute inset-0 pointer-events-none border-[30px] border-black/20">
              <div className="w-8 h-8 border-t-2 border-l-2 border-[var(--color-neon-cyan)] absolute top-4 left-4" />
              <div className="w-8 h-8 border-t-2 border-r-2 border-[var(--color-neon-cyan)] absolute top-4 right-4" />
              <div className="w-8 h-8 border-b-2 border-l-2 border-[var(--color-neon-cyan)] absolute bottom-4 left-4" />
              <div className="w-8 h-8 border-b-2 border-r-2 border-[var(--color-neon-cyan)] absolute bottom-4 right-4" />
           </div>
        </div>

        {/* Sidebar Findings */}
        <div className="space-y-6">
           <div className="glass p-6 rounded-[2.5rem] border border-white/5 h-full">
              <h4 className="text-xs font-mono uppercase text-gray-500 tracking-[0.2em] mb-6">Resultados de IA</h4>
              
              {analyzing ? (
                <div className="flex flex-col items-center justify-center h-64 gap-4">
                   <Loader2 size={32} className="animate-spin text-[var(--color-neon-cyan)]" />
                ...
                <div className="space-y-4">
                   {findings.map(f => (
                     <div key={f.id} className="p-4 bg-white/5 border border-white/5 rounded-2x hover:bg-white/10 transition-all cursor-pointer">
                   ...
                   <div className="pt-6 mt-6 border-t border-white/5">
                      ...
                      <p className="text-3xl font-black text-white">$250.000</p>
