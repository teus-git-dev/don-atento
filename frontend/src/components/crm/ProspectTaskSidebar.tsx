...
export default function ProspectTaskSidebar({ prospect, onClose, onRefresh }: ProspectTaskSidebarProps) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[400px] bg-[#0B1121] border-l border-white/10 z-[200] shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
      <div className="p-6 border-b border-white/10 bg-white/5 flex justify-between items-center">
        <div>
          <h3 className="text-lg font-bold text-white uppercase tracking-widest">Seguimiento Lead</h3>
          <p className="text-xs text-gray-400 font-mono mt-1">{prospect.firstName} {prospect.lastName}</p>
        </div>
        <button onClick={onClose} className="p-2 bg-white/5 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
          <X size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
            ...
            <div className="h-2 bg-white/5 rounded-full overflow-hidden">
               <div 
                 className="h-full bg-gradient-to-r from-[var(--color-neon-blue)] to-[var(--color-neon-cyan)] transition-all duration-500 shadow-[0_0_10px_rgba(0,255,255,0.3)]"
        ...
        {prospect.interestedProperties && prospect.interestedProperties.length > 0 && (
          <div className="space-y-4 animate-in fade-in duration-500 delay-200">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-[var(--color-neon-blue)]">Inmuebles de Interés</h4>
            <div className="grid gap-3">
              {prospect.interestedProperties.map((prop: any) => (
                <div key={prop.id} className="group p-3 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 hover:border-[var(--color-neon-cyan)]/30 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-[var(--color-neon-cyan)]/10 rounded-lg text-[var(--color-neon-cyan)]">
                      <Building2 size={16} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-white truncate">{prop.title}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <span className="px-1.5 py-0.5 bg-white/5 rounded text-[8px] font-mono font-bold text-gray-400 border border-white/10">
          ...
          <div className="space-y-3">
            ...
            {(!prospect.tasks || prospect.tasks.length === 0) && (
              <div className="py-10 text-center opacity-40">
                ...
                <p className="text-[10px] text-gray-400 uppercase font-mono tracking-widest">No hay tareas pendientes</p>
              </div>
            )}
          </div>
        </div>
      ...
      {prospect.status !== 'CLOSED_WON' && (
        <div className="p-6 border-t border-white/10 bg-[var(--color-neon-cyan)]/5 group hover:bg-[var(--color-neon-cyan)]/10 transition-all">
          <button 
            onClick={() => setIsContractModalOpen(true)}
            className="w-full flex items-center justify-between gap-3 text-[var(--color-neon-cyan)] font-bold text-xs uppercase tracking-widest"
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-[var(--color-neon-cyan)]/10 rounded-lg">
                <FileText size={18} />
              </div>
        ...
        <form onSubmit={addTask} className="space-y-3">
          ...
          <div className="flex gap-2">
             <div className="flex-1 relative">
                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 shadow-none" size={14} />
                <input 
                  type="date" 
                  value={newTaskDate}
                  ...
                  className="w-full bg-[#0B1121] border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-[10px] text-gray-300 focus:outline-none focus:border-[var(--color-neon-cyan)]/50"
             ...
             <button 
               ...
               className="bg-[var(--color-neon-cyan)] hover:bg-[var(--color-neon-cyan)]/80 text-black p-2.5 rounded-xl transition-all disabled:opacity-50 shadow-[0_0_10px_rgba(0,255,255,0.2)]"
             >
