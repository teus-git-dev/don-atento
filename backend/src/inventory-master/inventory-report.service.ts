  ...
  async generateInventoryPDF(propertyId: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const owner = property.relations.find(
        (r: any) => r.relationType === 'OWNER',
      )?.user;
      const tenant = property.relations.find(
        (r: any) => r.relationType === 'TENANT',
      )?.user;
      ...
      for (const zone of property.zones) {
        ...
        for (const item of zone.items as any[]) {
          doc
            ...
            .fontSize(9)
            .text(
              `${item.name} (${item.quantity || 1} ${item.material || ''}): `,
              70,
              undefined,
              { continued: true },
            );
          ...
          doc
            ...
            .fontSize(8)
            .text(
              `Notas: ${item.comments || item.description || 'Sin novedad'}`,
              80,
            );
          doc.moveDown(0.5);
        }
    ...
    if (type === 'CHECK_IN' || (type as any) === 'DELIVERY') {
      if (owner?.phone) {
        await this.whatsapp.sendMessage(
          owner.phone,
          `✨ Don IQ: Tu reporte de inventario (${type}) para ${property.address} ya está listo.`,
        );
      }
      if (tenant?.phone) {
        await this.whatsapp.sendMessage(
