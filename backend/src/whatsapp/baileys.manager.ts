...
export class BaileysManager implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly authBaseDir = path.join(process.cwd(), 'storage', 'baileys_sessions');

  // Callback para mensajes entrantes — se inyecta desde WhatsappModule
  private onMessageCallback: ((tenantId: string, from: string, text: string, mediaType?: string) => Promise<void>) | null = null;
   ...
   */
  setMessageHandler(
    handler: (tenantId: string, from: string, text: string, mediaType?: string) => Promise<void>,
  ) {
   ...
   */
  async connectTenant(tenantId: string): Promise<{ status: WhatsappConnectionStatus; qr?: string }> {
    ...
    const adapter = new BaileysAdapter(tenantId, this.authBaseDir, this.antiBan);

    // Escuchar eventos
    adapter.on('message', async (data: { from: string; text: string; mediaType?: string }) => {
   ...
   */
  getConnectionStatus(tenantId: string): { status: WhatsappConnectionStatus; qr?: string; health?: any } {
    const adapter = this.adapters.get(tenantId);
    if (!adapter) {
