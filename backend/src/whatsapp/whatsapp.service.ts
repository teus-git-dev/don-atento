...
import { TicketPriority, InteractionChannel, SentimentAnalysis, ProspectSource } from '@prisma/client';
...
export class WhatsappService {
  ...
  async processIncomingMessage(from: string, text: string) {
    ...
    if (currentTicket) {
      await this.cognitiveService.logInteraction(currentTicket.id, user?.id || null, text, InteractionChannel.WHATSAPP, sentiment);
      await this.cognitiveService.logInteraction(currentTicket.id, null, finalResponse, InteractionChannel.SYSTEM_AI, sentiment);
    } else if (prospect) {
      // Log for CRM purposes if not a ticket related message
      await this.crmService.addInteraction(prospect.id, text, InteractionChannel.WHATSAPP);
      await this.crmService.addInteraction(prospect.id, finalResponse, InteractionChannel.SYSTEM_AI);
    }
