...
export class MaintenancePredictorService {
  ...
  async calculatePropertyHealthScore(propertyId: string): Promise<{ score: number; status: string; recommendations: string[] }> {
    const property = await this.prisma.property.findUnique({
      where: { id: propertyId },
      include: {
        tickets: {
          where: { createdAt: { gte: new Date(Date.now() - 180 * 24 * 60 * 60 * 1000) } }, // Last 6 months
    ...
    if (!property) return { score: 100, status: 'HEALTHY', recommendations: [] };
    ...
    const oldItems = property.inventoryItems.filter(item => {
        ...
        return !item.lastInspectionDate || (Date.now() - new Date(item.lastInspectionDate).getTime() > 90 * 24 * 60 * 60 * 1000);
    }).length;
